...
namespace Curso.CustomEditor
{
    public class ItemGenerator : EditorWindow
    {
        ...
        private string _pathAndName = "Assets/Curso/10 Custom Editor/Item Data/New Item.asset"; //para guardar los nuevos datos del scriptable object

        [MenuItem("Tools/Item Generator")]        
        static void Init() //para crear el tool
        {
        ...
        {
            _data = CreateInstance<ItemSO>(); //creo una instancia al scriptableObject
            _data.title = title;
            _data.price = price;
            _data.sprite = sprite;
        }
